...
#define RF_PERIOD  20
//#define ARF001 1    //德辉达降本
#define ARF001 0  //PRO_MAX
#define LIS2DH 1  //2D陀螺仪
//#define LIS3DH 0  //3D陀螺仪
#define ENABLE_FREERTOS_HEAP_STACK_LOG 0
#define DEBUG_IDWG 1
#define AGEING_TEST 0  //用于续航测试
...
typedef struct
{
	...
	uint8_t   Period_Flag:1;         //周期标志位

	uint8_t   Heating_Flag:3;
	uint8_t   Receive_Flag:2;
	uint8_t   Mode_Switch_Flag:3;	

	uint8_t   MotionStateFlage:1;    //六轴传感器运动标志
	uint8_t   Check_Protect_Flage:1; //移动检测保护	
	uint8_t   Temp_Protect_Flage:5;  //过温停止保护	
