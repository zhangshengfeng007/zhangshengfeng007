...
#if ARF001				  // 德辉达降本
#define EMS_Freq 1000	  // 周期 时间
#define duty 30			  // 脉宽 30
#define interval 35		  // 上下脉宽间隔时间
...
#else
#define EMS_Freq 250	  // 周期 时间
#define duty 11			  // 脉宽
#define interval 30		  // 上下脉宽间隔时间
#define channel_p_start 5 //
#define channel_p_stop channel_p_start + duty
#define channel_n_start channel_p_stop + interval
#define channel_n_stop channel_n_start + duty
...
void Set_Ems_level(_Sys_Info_TypeDef *EmsLevel)
{
	...
	if (SysInfo.Power_Value.state == System_ON)
	{
		if (EmsLevel->WorkState == upkeep_mode)
		{
			...
			{
				EMS_Handle.EMS_Cnt++;
#if EMS_Mode // EMS???2s?????仯 EMS_Handle.vol_value+1V -> EMS_Handle.vol_value-> EMS_Handle.vol_value-1V ???
...
#else
				if (EMS_Handle.EMS_Cnt > 50) // EMS变频 1 KHZ ~ 4 KHZ
				{
					EMS_Handle.EMS_Cnt = 0;
					EMS_PSC++;
					//						EMS_PSC=0; //????
				...
				//					if(EMS_Handle.EMS_Cnt<100)//EMS变频 1 KHZ ~ 3 KHZ
				//					{
				//						EMS_PSC=0;
				//					}
...
void Ems_DeInit(void)
{
	...
	PID.Flag = 0;
	//	VBAT_DET_OFF(); //使能 V33V5020
	//	VBAT_OUT_OFF(); //
	EMS_Pwm_Stop();
}
/**************************************************************************************
 * FunctionName   : Touch_Skin_EMS_Freq_Conversion(void)
 * Description    : 皮肤接触后，EMS脉宽缓慢升高
 * EntryParameter : None
 * ReturnValue    : None
 **************************************************************************************/
...
int PID_IncreaseCalc(_PID_HANDLE_TypeDef *vPID)
{
	...
	vPID->CurErr = vPID->TargetVal - vPID->CurrentVal; // 当前偏差
	...
	vPID->LastErr = vPID->CurErr; // 当前偏差变成上一次偏差
