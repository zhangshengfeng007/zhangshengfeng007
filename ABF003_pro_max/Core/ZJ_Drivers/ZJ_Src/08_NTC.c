 ...
 * ReturnValue    : None
 **************************************************************************************/
void NTC_Temp_Running(void) // 1ms运行一次
{
	//	int8_t current_temp;
...
//  static uint8_t OverTemp=42,StopTemp=45,NormalTemp =40;
	...
	Rt_Value = ((5110 - ntc_value) * 3600 / ntc_value) - 3600;										   // 根据同一回路，电流相等，计算得出NTC电阻值

	SysInfo.NTC_Temp = Get_Temp(Rt_Value, 10000, 3380); // 10K B=3380
	NTC_Temp_compensation(&SysInfo, 0.042);				// 温度补偿

  if(SysInfo.Temp_Protect_Flage) //开机及离开皮肤10S，过温温度减1°
	{
		SysInfo.NTC_Temp += 1;
