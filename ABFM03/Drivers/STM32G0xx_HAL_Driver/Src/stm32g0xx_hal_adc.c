...
HAL_StatusTypeDef HAL_ADC_Init(ADC_HandleTypeDef *hadc)
{
     ...
     )
  {
    ...
    /*       (in this case, flag Read_ADC_Flag_RDY is not set).                    */
...
HAL_StatusTypeDef HAL_ADC_DeInit(ADC_HandleTypeDef *hadc)
{
  ...
  /* Reset register ISR */
  __HAL_ADC_CLEAR_FLAG(hadc, (Read_ADC_Flag_AWD3  | Read_ADC_Flag_AWD2 |
                              Read_ADC_Flag_AWD1  | Read_ADC_Flag_OVR  |
                              Read_ADC_Flag_EOS   | Read_ADC_Flag_EOC  |
                              Read_ADC_Flag_EOSMP | Read_ADC_Flag_RDY));
...
HAL_StatusTypeDef HAL_ADC_Start(ADC_HandleTypeDef *hadc)
{
  ...
  if (LL_ADC_REG_IsConversionOngoing(hadc->Instance) == 0UL)
  {
    ...
    if (tmp_hal_status == HAL_OK)
    {
      ...
      __HAL_ADC_CLEAR_FLAG(hadc, (Read_ADC_Flag_EOC | Read_ADC_Flag_EOS | Read_ADC_Flag_OVR));
...
HAL_StatusTypeDef HAL_ADC_PollForConversion(ADC_HandleTypeDef *hadc, uint32_t Timeout)
{
  ...
  {
    tmp_flag_end = Read_ADC_Flag_EOS;
  }
  ...
  else /* ADC_EOC_SINGLE_CONV */
  {
    ...
    {
      tmp_flag_end = (Read_ADC_Flag_EOC);
    }
  }
     ...
     )
  {
    ...
    if (__HAL_ADC_GET_FLAG(hadc, Read_ADC_Flag_EOS))
    {
  ...
  if (hadc->Init.LowPowerAutoWait == DISABLE)
  {
    ...
    __HAL_ADC_CLEAR_FLAG(hadc, (Read_ADC_Flag_EOC | Read_ADC_Flag_EOS));
  }
...
/**
  ...
  * @param Timeout Timeout value in millisecond.
  * @note   The relevant flag is cleared if found to be set, except for Read_ADC_Flag_OVR.
  *         Indeed, the latter is reset only if hadc->Init.Overrun field is set
  *         to ADC_OVR_DATA_OVERWRITTEN. Otherwise, data register may be potentially overwritten
  *         by a new converted data as soon as OVR is cleared.
  *         To reset OVR flag once the preserved data is retrieved, the user can resort
  *         to macro __HAL_ADC_CLEAR_FLAG(hadc, Read_ADC_Flag_OVR);
  * @retval HAL status
  */
HAL_StatusTypeDef HAL_ADC_PollForEvent(ADC_HandleTypeDef *hadc, uint32_t EventType, uint32_t Timeout)
{
  ...
  switch (EventType)
  {
    ...
    case ADC_EOSMP_EVENT:
      ...
      __HAL_ADC_CLEAR_FLAG(hadc, Read_ADC_Flag_EOSMP);
    ...
    case ADC_AWD_EVENT:
      ...
      __HAL_ADC_CLEAR_FLAG(hadc, Read_ADC_Flag_AWD1);
    ...
    case ADC_AWD2_EVENT:
      ...
      /* Clear ADC analog watchdog flag */
      __HAL_ADC_CLEAR_FLAG(hadc, Read_ADC_Flag_AWD2);
    ...
    case ADC_AWD3_EVENT:
      ...
      /* Clear ADC analog watchdog flag */
      __HAL_ADC_CLEAR_FLAG(hadc, Read_ADC_Flag_AWD3);
    ...
    default: /* Case ADC_OVR_EVENT */
      ...
      else
      {
        /* Clear ADC Overrun flag only if Overrun is set to ADC_OVR_DATA_OVERWRITTEN
           ...
           as OVR is cleared. */
        __HAL_ADC_CLEAR_FLAG(hadc, Read_ADC_Flag_OVR);
      }
      break;
  }
...
/**
  ...
  *         mode), the user must:
  *          1. first clear the EOSMP flag if set with macro __HAL_ADC_CLEAR_FLAG(hadc, Read_ADC_Flag_EOSMP)
...
/**
  ...
  *         or @ref __HAL_ADC_CLEAR_FLAG(&hadc, Read_ADC_Flag_EOS).
  * @param hadc ADC handle
...
void HAL_ADC_IRQHandler(ADC_HandleTypeDef *hadc)
{
  ...
  if (((tmp_isr & Read_ADC_Flag_EOSMP) == Read_ADC_Flag_EOSMP) && ((tmp_ier & ADC_IT_EOSMP) == ADC_IT_EOSMP))
  {
  ...
  if ((((tmp_isr & Read_ADC_Flag_EOC) == Read_ADC_Flag_EOC) && ((tmp_ier & ADC_IT_EOC) == ADC_IT_EOC)) ||
      (((tmp_isr & Read_ADC_Flag_EOS) == Read_ADC_Flag_EOS) && ((tmp_ier & ADC_IT_EOS) == ADC_IT_EOS)))
  {
  ...
  /* ========== Check Analog watchdog 1 flag ========== */
  if (((tmp_isr & Read_ADC_Flag_AWD1) == Read_ADC_Flag_AWD1) && ((tmp_ier & ADC_IT_AWD1) == ADC_IT_AWD1))
  {
    /* Set ADC state */
    SET_BIT(hadc->State, HAL_ADC_STATE_AWD1);
  ...
  /* ========== Check analog watchdog 2 flag ========== */
  if (((tmp_isr & Read_ADC_Flag_AWD2) == Read_ADC_Flag_AWD2) && ((tmp_ier & ADC_IT_AWD2) == ADC_IT_AWD2))
  {
    /* Set ADC state */
    SET_BIT(hadc->State, HAL_ADC_STATE_AWD2);
  ...
  /* ========== Check analog watchdog 3 flag ========== */
  if (((tmp_isr & Read_ADC_Flag_AWD3) == Read_ADC_Flag_AWD3) && ((tmp_ier & ADC_IT_AWD3) == ADC_IT_AWD3))
  {
    /* Set ADC state */
    SET_BIT(hadc->State, HAL_ADC_STATE_AWD3);
  ...
  /* ========== Check Overrun flag ========== */
  if (((tmp_isr & Read_ADC_Flag_OVR) == Read_ADC_Flag_OVR) && ((tmp_ier & ADC_IT_OVR) == ADC_IT_OVR))
  {
  ...
  if (((tmp_isr & Read_ADC_Flag_CCRDY) == Read_ADC_Flag_CCRDY) && ((tmp_ier & ADC_IT_CCRDY) == ADC_IT_CCRDY))
  {
    /* Channel configuration ready callback */
    HAL_ADCEx_ChannelConfigReadyCallback(hadc);

    /* Clear ADC analog watchdog flag */
    __HAL_ADC_CLEAR_FLAG(hadc, Read_ADC_Flag_CCRDY);
  }
}
...
HAL_StatusTypeDef ADC_Enable(ADC_HandleTypeDef *hadc)
{
  ...
  if (LL_ADC_IsEnabled(hadc->Instance) == 0UL)
  {
    ...
    if (hadc->Init.LowPowerAutoPowerOff != ENABLE)
    {
      ...
      while (__HAL_ADC_GET_FLAG(hadc, Read_ADC_Flag_RDY) == 0UL)
      {
        ...
        {
          /* New check to avoid false timeout detection in case of preemption */
          if (__HAL_ADC_GET_FLAG(hadc, Read_ADC_Flag_RDY) == 0UL)
          {
            /* Update ADC state machine to error */
            SET_BIT(hadc->State, HAL_ADC_STATE_ERROR_INTERNAL);
...
HAL_StatusTypeDef ADC_Disable(ADC_HandleTypeDef *hadc)
{
     ...
     )
  {
    ...
    if ((hadc->Instance->CR & (ADC_CR_ADSTART | ADC_CR_ADEN)) == ADC_CR_ADEN)
    {
      /* Disable the ADC peripheral */
      LL_ADC_Disable(hadc->Instance);
      __HAL_ADC_CLEAR_FLAG(hadc, (Read_ADC_Flag_EOSMP | Read_ADC_Flag_RDY));
    }
    else
    {
      /* Update ADC state machine to error */
