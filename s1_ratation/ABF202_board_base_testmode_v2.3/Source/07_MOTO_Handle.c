...
{
  if(Moto_Control.Electric_Machinery_Onoff == 1) ///电机振动
  {
    MOTO_ENABLE;
  }
  else
...
//================================================================================================
// Descriptions:      蜂鸣器鸣叫函数 长滴一声（0.5S）
// Input parameters:        control  1 打开蜂鸣器工作0.5s  0关闭
// Output parameters:  
// Returned value:
// Created by:        zhangx
...
void buzzer0_5(uint8_t *control,uint8_t *last)
{
  ...
  if((HAL_GetTick() - buzzertime) > 400) ///0.5s后关闭蜂鸣器	
  {
    if(*control != 0)
    {
...
//================================================================================================
// Descriptions:      蜂鸣器鸣叫函数 长滴一声（0.1S）
// Input parameters:        control  1 打开蜂鸣器工作1.0s  0关闭
// Output parameters:  
// Returned value:
// Created by:        zhangx
// Created date:      2021.05.27
...
void buzzer1_0(uint8_t *control,uint8_t *last)
{
  ...
  if((HAL_GetTick() - buzzertime) > 200) ///1.0s后关闭蜂鸣器	
  {
    if(*control != 0)
    {
      Moto_Control.Electric_Machinery_Onoff = 0;
...
void buzzerChirp(uint8_t slide_mode,uint8_t mode, uint8_t Seal_gear,uint8_t Slide_gear,uint8_t onoff,uint8_t load,uint8_t color)
{
  ...
  {
    //    if(lastonoff != onoff)									//如果开关机有变化，振动
    //    {
    ...
    if(lastSlidegear!= Slide_gear)										//如果档位有变化 振动
    {
    ...
    if(lastload == 0)
    {
      if(load == 1)                       // 如果接入负载 振动
      {
        // if(Key_State.Mode_choice_key == SEAL_MODE)///盖章模式
        // {
        //   buzzer_load_flage = 1;
        //   last_load_flage = 0;
        // }
