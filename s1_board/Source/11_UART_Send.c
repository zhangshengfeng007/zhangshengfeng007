...
//      HAL_UARTEx_ReceiveToIdle_DMA(&huart2, recv_data_buff, sizeof(recv_data_buff));//HAL_UART_Receive_DMA(&huart2, recv_data_buff,sizeof(recv_data_buff));//������һ�ֽ���sizeof(recv_data_buff)  rx_len
...
{
	if(huart->Instance == USART2)  //�ж��Ƿ���UART2
	 {
			Sys_Info.Test_Mode.Receive_Flag = 1;         //��1�������ս���
			HAL_UART_DMAStop(&huart2);
			rx_len =Size;              //���յ������ݳ���
	 }
}
...
//				SYS_BUS_SW_OFF;
      ...
      case Comm_Batter_Value :
	  	uint32_t tmp;
		tmp = bat_data.adc_val * 5000 / 4096;
        TX_Command_Handle(Comm_Batter_Value, tmp);
...
//				Sys_Info.PowerOn_Time_Counter = 21;
				...
				break;
      case Comm_RF_Test :
					if(datacom[rx_len-4]==0xFF && datacom[rx_len-3]==0xFF) //�ر�RF//Length
					{
...
//					Sys_Info.Test_Mode.RF_EMSTEST_Flag = 1;
				...
				break;
      case Comm_EMS_Test :
				if(datacom[rx_len-4]==0xFF && datacom[rx_len-3]==0xFF) //�ر�EMS Length
				{
						Sys_Info.Power_State = MACHINEOFF;
						LOAD_BUS_SW_OFF;
						EMS_OUT_Stop();
...
void Aging_Test(void)
{
	if(Sys_Info.Test_Mode.TestMode == Aging_TestModel)//Aging_Model
	{
		...
		{
			if((bat_data.disp_level == Bat_Level_Full))//(bat_data.adc_val > 4120)&&&&(bat_data.disp_level == Bat_Level_Full)//���ѳ���������ŵ�ģʽ//if(BAT_CHARGE_STATE_IN() == 1)//���ѳ���������ŵ�ģʽ 3900))//  4180  &&(BAT_CHARGE_STATE_IN() == 1)
			{
				Sys_Info.Test_Mode.chargetimes++;
				if(Sys_Info.Test_Mode.chargetimes < 2)//2 ������һ�ε�
				{
	...
	//				Sys_Power_On();
				...
				else
				{
					...
					OVP_ENABLE;
					Sys_Info.Work_Mode = NORMAL;//�Զ��˳��ò���ģʽ
					Sys_Info.Power_State = MACHINEOFF;
				}
			}
		}
		if((bat_data.adc_val < BAT_3300mV_ADC_VAL) || Sys_Info.Test_Mode.First_charge_Flag)//���ѷŹ⣬������ģʽ 3100
		{
		...
		{
			if(bat_data.adc_val > BAT_4100mV_ADC_VAL)
			{
				BAT_CHARGE_DISABLE;
