...
uint16_t Get_LIS3DH_Read(uint16_t *ADC_Source, uint16_t Counts)
{
  uint8_t i;
...
uint16_t LIS3DH_Data_Filter(uint16_t *pData, uint8_t channel, uint8_t len)
{
  ...
  return value_buf1[(10 - 1) / 2];
}

void LIS3DH_Data_Smooth(void)
{
  static uint16_t AXIS_Counts;
  static uint16_t AXIS_X[10], AXIS_Y[10], AXIS_Z[10];

  LIS3DH_GetAccAxesRaw(&ddata); // ��ȡ���ٶȳ�ʼֵ

  if (AXIS_Counts < 10)
  {
    AXIS_X[AXIS_Counts] = ddata.AXIS_X;
    AXIS_Y[AXIS_Counts] = ddata.AXIS_Y;
    AXIS_Z[AXIS_Counts] = ddata.AXIS_Z;
    AXIS_Counts++;
  }
  else
  {
    AXIS_Counts = 0;
    //		ddata.AXIS_X = Get_LIS3DH_Read(AXIS_X,10);
    //		ddata.AXIS_Y = Get_LIS3DH_Read(AXIS_Y,10);
    //		ddata.AXIS_Z = Get_LIS3DH_Read(AXIS_Z,10);

    ddata.AXIS_X = LIS3DH_Data_Filter(AXIS_X, 0, 10);
    ddata.AXIS_Y = LIS3DH_Data_Filter(AXIS_Y, 0, 10);
    ddata.AXIS_Z = LIS3DH_Data_Filter(AXIS_Z, 0, 10);
  }
}
status_t LIS3DH_get_angle(float acc_x, float acc_y, float acc_z)
{
  ...
  /*
    ...
    gdata.AXIS_X = angle_x;
    gdata.AXIS_Y = angle_y;
    gdata.AXIS_Z = angle_z;
  ...
  */
  ...
  /*
    ...
    jdata.AXIS_X = angle_x;
    jdata.AXIS_Y = angle_y;
