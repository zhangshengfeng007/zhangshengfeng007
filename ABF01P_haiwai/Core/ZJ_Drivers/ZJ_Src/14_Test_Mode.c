...
void Test_Mode_Key_Process(void)
{
  ...
  if (!SysInfo.Test_Mode.Quit_Flag && (SysInfo.Test_Mode.Test_Mode_Flag == OFF)) // USB����ǰ30s�������������ģʽ
  {
    SysInfo.Test_Mode.Quit_Test_30s_Cnt++;
    if (++SysInfo.Test_Mode.Quit_Test_30s_Cnt >= 3000) // ����30s�޲���
    {
  ...
  if (SysInfo.Batt_Value.Usb_flag && (SysInfo.Test_Mode.Test_Mode_Flag != ON))
  {
    ...
    {
      if ((Key_Plus_Read_In == GPIO_PIN_RESET) && (Key_minus_Read_In == GPIO_PIN_RESET)) // ���UP&DWN����3S
      {
        Test_timer_cnt++;
        ...
        if (Test_timer_cnt > 300)
        {
          Enter_Test_Flag = 3;
          Test_timer_cnt = 0;
          SysInfo.Test_Mode.Test_Mode_Flag = Standy; // �������ģʽ
    ...
    else if (3 == Enter_Test_Flag)
    {
      ...
      if (Test_timer_cnt < 300)
      {
        if (Key_Plus_Read_In && Key_minus_Read_In) // ���UP&DWN���ɿ�
        {
          Enter_Test_Flag = 4;
          Test_timer_cnt = 0;
          Key_Cnt = 0;
    ...
    else if (4 == Enter_Test_Flag)
    {
      ...
      if (Test_timer_cnt < 6000)
      {
        ...
        if (Key_Cnt >= 4 && Key_Scap_Read_In) // ���˶̰�KEY_ON����⵽���������˳�
        {
          Enter_Test_Flag = 0x5a;
          SysInfo.Test_Mode.Test_Mode_Flag = ON; // �������ģʽ
          SysInfo.Test_Mode.Test_Mode = 0;
        }
      }
      else
...
void Test_LED_Display_Process(void)
{
  ...
  if (SysInfo.Test_Mode.Test_Mode_Flag == Standy || SysInfo.Test_Mode.Twink_Flag == 0x04) // ģʽ����˸
  {
    Lock_Flag = 1;
    if (Led_Flag)
    {
 ...
 * Description    : ///��������ʾ�з���������ԭ�룩bit6~bit0=OLED~WLED5��bit6����λ
 * EntryParameter : None
 * ReturnValue    : None
 **************************************************************************************/
...
uint8_t display_binary(int8_t bin_data)
{
  ...
  if (bin_data & 0x80) // ����
  {
    display_bin |= 0x40;      // ��Ƿ���λ
    temp = (uint8_t)bin_data; // ǿ�ƶ�����ת��
    temp &= 0x7f;             // �������λ
    temp = (~temp) + 1;       // ��ԭ��
    display_bin |= temp;
  }
  ...
  if (SysInfo.Test_Mode.Test_Mode_Flag != OFF)
  {
    ...
    if (!SysInfo.Test_Mode.Ageing_Flag)
    {
      ...
      case 5: /***********************����ģʽ,�缫��ͨ�Զ����ģʽ***********************/
        ...
        if (Rotate_Count > 5 && Pole_Connect_Flag) // �伫��50ms�жϵ缫��ͨ
        {
          switch (Pole_Counts)
          {
          case 0:
...
uint8_t Auto_Continuity_Test_Process(void)
{
  ...
  if (AutoRotate_Count > 5 && AutoPole_Connect_Flag) // �伫��50ms�жϵ缫��ͨ
  {
...
void Test_AutoMode_Run_Process(void)
{
  ...
  if (SysInfo.Test_Mode.Test_Mode_Flag == Auto_ON)
  {
    ...
    case 2: /***********************RF���μ��ģʽ***********************/
      ...
      {
        if (++SysInfo.Test_Mode.Auto_Mode_Cnt > 500) // 5sʱ�䵽
        {
          //							Auto_Mode_Level =3;
          SysInfo.Test_Mode.Auto_Mode_Cnt = 0;
          RF_Osc_Off();
    ...
    case 3: /***********************EMS���μ��ģʽ***********************/
      ...
      break;
    default:
      if (++SysInfo.Test_Mode.Auto_Mode_Cnt > 15000) // 150sʱ�䵽
      {
        SysInfo.Test_Mode.Auto_Mode_Cnt = 0;
...
void Test_UART_Deal_Process(void)
{
  ...
  if (Test_Flag)
  {
    ...
    case Comm_Inset_Testmode:
      if (SysInfo.Test_Mode.Data[4] == 0x00 && SysInfo.Test_Mode.Data[5] == 0x00) // ������������ģʽָ��
      {
        SysInfo.Test_Mode.Test_Mode_Flag = Auto_ON;
        BOOST_5V_ON();
      }
      ...
      break;
    case Comm_manufacture_Info: // ��ȡ������Ϣ���汾��
      Length = 0x0C;
    ...
    case Comm_HeadLED_Control:
      ...
      break;
    case Comm_Electrode_Test: // �缫ͷ��ͨ�Բ���
      ...
      SysInfo.Test_Mode.Twink_Flag = 0x04; // ģʽ����˸
      Set_Mbi5020_Out(RF_CH1_ON_BIT);
      Test_RF_Waveform_Process();
      break;
    case Comm_RF_Control:                                                                           // RF��Ƶ����
      if (SysInfo.Test_Mode.Data[Length - 4] == 0xFF && SysInfo.Test_Mode.Data[Length - 3] == 0xFF) // �ر�RF
      {
        SysInfo.Test_Mode.Auto_Mode_Level = 0x5a;
        SysInfo.Test_Mode.Level = 0;
        RF_Call_Data(); // �ر�RF��EMS
      ...
      break;
    case Comm_EMS_Control:                                                                          // ����EMS����
      if (SysInfo.Test_Mode.Data[Length - 4] == 0xFF && SysInfo.Test_Mode.Data[Length - 3] == 0xFF) // �ر�EMS
      {
        SysInfo.Test_Mode.Auto_Mode_Level = 0x5a;
        SysInfo.Test_Mode.Level = 0;
        RF_Call_Data(); // �ر�RF��EMS
    ...
    case Comm_KEY_Control:                                                                          // ���밴������
      if (SysInfo.Test_Mode.Data[Length - 4] == 0xFF && SysInfo.Test_Mode.Data[Length - 3] == 0xFF) // �رհ�������
      {
        SysInfo.Test_Mode.Auto_Mode_Level = 0x5a;
        SysInfo.Test_Mode.Test_Mode_Flag = OFF;
      }
      ...
      break;
    case Comm_Light_Control:                                                                        // ����ƹ����
      if (SysInfo.Test_Mode.Data[Length - 4] == 0xFF && SysInfo.Test_Mode.Data[Length - 3] == 0xFF) // �رյƹ����
      {
        SysInfo.Test_Mode.Auto_Mode_Level = 0x5a;
        VBAT_OUT_OFF();
        IRled_stop();
      ...
      break;
    case Comm_Aging_Model:                                                                          // �ϻ�����
      if (SysInfo.Test_Mode.Data[Length - 4] == 0xFF && SysInfo.Test_Mode.Data[Length - 3] == 0xFF) // �ر��ϻ�����
      {
    ...
    case Comm_Inset_Life_Testl:                                                                     // ��������
      if (SysInfo.Test_Mode.Data[Length - 4] == 0xFF && SysInfo.Test_Mode.Data[Length - 3] == 0xFF) // �ر���������
      {
        Ageing_TestData_DeInit();
    ...
    case Comm_Inset_Bootloader: // ����Bootloader����
      UART_Send_Data(SysInfo.Test_Mode.Data, Length);
      BKP_SRAM_Init();
      HAL_NVIC_SystemReset();
      break;
    case Comm_Check_Version: // �汾У��

      break;
    case Comm_Standby:                                                                              // ���������������ģʽ
