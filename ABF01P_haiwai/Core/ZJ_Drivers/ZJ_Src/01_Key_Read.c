...
void Key_S1_Str(void) //
{
	if (SysInfo.Power_Value.state == System_ON || SysInfo.Power_Value.state == System_Standy)
	{
		...
		SysInfo.StayTime = 50;
		SysInfo.Skin_Touch_Montor_Flag = 1; // RF??????????ν????????????????λ
		...
		SysInfo.Freq_Cnt = 0;
		if (SysInfo.Power_Value.state == System_Standy) // ?????????????л??????????
		{
			SysInfo.Heating_Flag = 0x01; // ?????????5?????У???缫??????±??λ
		}
		SysInfo.Mode_Switch_Flag = 0x01; // RF??????
		SysInfo.Reminder_Cnt = 0;
		SysInfo.Temp_Protect_Flage = 1; //开机过温温度减2℃标志位
	}
	else // ???????
	{
		VBAT_DET_ON();							   // ???ADC??????
...
void Key_S1_Long_Str(void)
{
	...
	if (SysInfo.Power_Value.state == System_OFF)
	{
		if ((SysInfo.Batt_Value.State == BAT_00_00_STATUS) || (SysInfo.Save_Data.BattState == BAT_00_00_STATUS))
		{
			...
			Led_Value.Mode = Batt_Low_0; // ??磬1??LED??1Hz??????5??
			Led_Value.Counts = 0;
		...
		else
		{
			...
			SysInfo.Montor_Flag = 1; // ???????
			SysInfo.StayTime = 50;
			HAL_NVIC_DisableIRQ(EXTI0_1_IRQn); // ??????ж?KEY_ON_OFF_Pin???
      SysInfo.Temp_Protect_Flage = 1; //开机过温温度减2℃标志位
		}
		SysInfo.StayTime_30s_Flag = 1; // ?л???λ??,????λ???30s,??????????????????л???λ?????1??????
	}
	else
	{
		//		Write_Parameter_To_Eeprom(); //??????浵λ
		RF_Osc_Off();
		RF_Stop();
		Set_Mbi5020_Out(0);
		IRled_stop();
		...
		HAL_NVIC_EnableIRQ(EXTI0_1_IRQn); // ???KEY_ON_OFF_Pin?ж?
		SysInfo.Reminder_Cnt = 0;
	}
	SysInfo.Sleep_Counts = 0;
}
